-- ServiceCard database schema: tables, views, RPCs, triggers, and Row Level Security for the automotive NFC companion.
--
-- This file is the consolidated, readable reference. The applied artifact is
-- supabase/migrations/. Keep the two in sync (tasks.md T149a).
--
...
--
-- The absence of an UPDATE or DELETE policy above already stops any RLS-bound
-- role from rewriting a revision. But Supabase grants ALL on public tables to
-- anon, authenticated and service_role by default, and service_role holds
-- BYPASSRLS — so without this revoke, a server-side code path holding the
-- service key could quietly erase recorded history. A buyer reading a shared
-- passport is entitled to a record that cannot be altered, and "we don't write
-- that query" is a convention rather than a guarantee.
